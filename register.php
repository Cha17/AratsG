...
if (isset($_SESSION['logged_in']) && $_SESSION['logged_in'] == true) {
  ...
  VALUES ($userid,$evalue,'$program','$year','$sec','$payment', '$note')";
    ...
    <div class="grid grid-cols-2 gap-16 mt-20">
      <div>
        <div class="px-20 ml-10">
          <h1 class="text-center text-[70px] pb-2">
            <?php
             $q = "SELECT * from events where `event-id`=$evalue";
             $result = mysqli_query($conn, $q);
             $row = mysqli_fetch_assoc($result); 
             echo $row['title'];?>
          </h1>
          <p class="text-justify pb-4 indent-8">
            <?php echo $row['addtl_info'];?>
          </p>
